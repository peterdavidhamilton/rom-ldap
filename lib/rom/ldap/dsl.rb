  ...
  module LDAP
    module DSL
      def equals(args)
        ...
        @ast << [:op_equal, attribute, escape(value)]
        self
      end
      alias where equals
      ...
      def unequals(args)
        attribute, value = args.to_a.flatten
        @ast << [:con_not, [:op_equal, attribute, escape(value)]]
        self
      end

      def present(attribute)
        @ast << [:op_equal, attribute, :wildcard]
        self
      end
      alias has present
      ...
      def missing(args)
        ...
        @ast << [:con_not, [:op_equal, attribute, :wildcard]]
        self
      end
      alias hasnt missing

      def gt(args)
        @ast << [:op_gt, *args.to_a.flatten]
        self
      end
      alias above gt

      def gte(args)
        @ast << [:op_gt_eq, *args.to_a.flatten]
        self
      end

      def lt(args)
        @ast << [:op_lt, *args.to_a.flatten]
        self
      end
      alias below lt

      def lte(args)
        @ast << [:op_lt_eq, *args.to_a.flatten]
        self
      end
