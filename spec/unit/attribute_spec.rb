RSpec.describe ROM::LDAP::Attribute do
  ...
  with_vendors do
    ...
    it '#definition' do
      expect(animals[:family].definition).to include('1.3.6.1.4.1.18055.0.4.1.2.1003')
      expect(animals[:family].definition).to include("NAME 'family'")
      expect(animals[:family].definition).to include("DESC 'The family classification of the animal'")
    end
    ...
    it '#editable?' do
      ...
      when 'open_dj'
        expect(animals[:family].editable?).to be(false)
      end
    end
