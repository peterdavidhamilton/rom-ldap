RSpec.describe ROM::LDAP::Relation, '#order' do
  ...
  with_vendors do
    ...
    context 'with strings' do
      ...
      before do
        %w[aardvark kakapo zebra].each { |w| factories[:animal, species: w] }
      end

      specify 'in alphabetical order' do
        expect(animals.order(:species).project(:species).to_a).to eql([
          { species: 'aardvark' },
          { species: 'kakapo' },
          { species: 'zebra' }
        ])
      ...
      specify 'in reverse alphabetical order' do
        expect(animals.order(:species).reverse.project(:species).to_a).to eql([
          { species: 'zebra' },
          { species: 'kakapo' },
          { species: 'aardvark' }
        ])
    ...
    context 'with booleans' do
      ...
      specify 'first false then true values' do
        ...
        skip 'Vendor does not order BOOLEAN' if %w'open_dj open_ldap'.include?(vendor)
